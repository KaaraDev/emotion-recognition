...
# ----------------------------- Feature Whitelist ------------------------------
# Nur DIESE Features werden verwendet (aus der Nutzerliste)
...
class CaseRandomForestClassifier:
    ...
    def __init__(
            ...
            allowed_features: Optional[List[str]] = None,  # <—— NEU: Feature-Whitelist
    ):
        ...
        self.allowed_features: List[str] = list(allowed_features or ALLOWED_FEATURES)
    ...
    def _filter_to_allowed_features(self, X: pd.DataFrame) -> pd.DataFrame:
        """Nur Whitelist-Features behalten; Reihenfolge fixieren; robust ggü. fehlenden."""
        cols_present = [c for c in self.allowed_features if c in X.columns]
        missing = [c for c in self.allowed_features if c not in X.columns]
        extra = [c for c in X.columns if c not in self.allowed_features]
        if missing:
            print(f"[FEATURES] Fehlen in Daten und werden ignoriert: {missing}")
        if extra:
            print(f"[FEATURES] Nicht erlaubt, werden gedroppt: {extra[:10]}{' …' if len(extra) > 10 else ''}")
        if not cols_present:
            raise ValueError("Nach dem Filtern bleiben keine Features übrig. Prüfe Spaltennamen.")
        X_f = X.loc[:, cols_present].copy()
        X_f.columns = cols_present  # Reihenfolge fixieren
        return X_f
    ...
    def fit(self, verbose: bool = True) -> RandomForestClassifier:
        ...
        if verbose:
            n, d = self.X_.shape
            print(f"[FIT] RandomForestClassifier trainiert auf {n} Fenstern mit {d} Features (Whitelist).")
        return self.model

    # --------------------------- Inferenz / Utils ---------------------------

    def _ensure_feature_order(self, X_new: pd.DataFrame | np.ndarray) -> np.ndarray:
        """Bringt eingehende Daten auf die Trainings-Spaltenreihenfolge."""
        if isinstance(X_new, pd.DataFrame):
            # Fehlende Spalten → Fehler, zusätzliche → ignorieren
            missing = [c for c in self.X_.columns if c not in X_new.columns]
            if missing:
                raise ValueError(f"Eingabedaten fehlen Spalten: {missing}")
            X_use = X_new.loc[:, list(self.X_.columns)].values
        else:
            X_use = X_new
        return X_use
    ...
    def predict(self, X_new: pd.DataFrame | np.ndarray) -> np.ndarray:
        ...
        Xv = self._ensure_feature_order(X_new)
    ...
    def load(path: str | bytes | "os.PathLike[str]") -> "CaseRandomForestClassifier":
        ...
        obj = CaseRandomForestClassifier(
            ...
            allowed_features=bundle.get("allowed_features", ALLOWED_FEATURES),
        )
    ...
    def load_prepared_from_dir(
        ...
        filtert strikt auf die Whitelist-Features,
        ...
        X_raw = df.drop(columns=[c for c in df.columns if c in drop_cols]).copy()

        # 3) Nur erlaubte Features behalten
        X = self._filter_to_allowed_features(X_raw)

        # 4) Videos global entfernen (Blue/Start/End etc.)
        _ = self._labels_from_meta(meta)  # nur Validierung
        X, meta = self._apply_global_video_filter(X, meta)

        # 5) Klassenlabels aus Video→Emotion (emo_map) NACH Filterung
        y_str = self._labels_from_meta(meta)
        le = LabelEncoder()
        y_int = le.fit_transform(y_str)

        # 6) Subjekt-Holdout abtrennen
        X, y_int, y_str, meta = self._split_holdout_by_subject(X, y_int, y_str, meta)

        # 7) Im Wrapper parken
        self.X_, self.y_int_, self.y_str_, self.meta_ = X, y_int, y_str, meta
        self.le_ = le
        print(f"[READY] TRAIN: X={X.shape}, y={y_int.shape}, meta={meta.shape} | Features={list(X.columns)}")
        return X, y_int, meta
...
if __name__ == "__main__":
    ...
    clfw = CaseRandomForestClassifier(
        preprocessor=None,  # wir laden aus Dateien
        rf_config=rf_cfg,
        emo_map=EMO_MAP,
        remove_videos={10, 11, 12},  # global raus (Blue/Start/End)
        exclude_subjects={30},  # echter Holdout (Subjekt 30)
        class_weight="balanced",
        allowed_features=ALLOWED_FEATURES,  # <—— nur Whitelist-Features
    )
    ...
    clfw.save("rf_emotion_classifier_from_saved_features_wlf.joblib")
