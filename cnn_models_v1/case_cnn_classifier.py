...
#
# 1D-CNN auf rohen physiologischen DAQ-Signalen.
...
class TrainCfg:
    ...
    # Ziel-Samplingrate nach Downsampling der DAQ-Daten
    target_fs: float = 50.0  # 50 Hz statt ~1000 Hz
...
# ---------------------------------------------------------------------
# Hilfsfunktionen: DAQ-Downsampling & Segmente
# ---------------------------------------------------------------------


def compute_downsample_factor(daqtime_sec: np.ndarray, target_fs: float) -> int:
    if len(daqtime_sec) < 2:
        return 1
    dt = np.diff(daqtime_sec)
    median_dt = np.median(dt)
    if median_dt <= 0:
        return 1
    orig_fs = 1.0 / median_dt
...
def build_windows_from_raw_and_interp(cfg: TrainCfg, logger: logging.Logger) -> Dict[str, np.ndarray]:
    """
    Lädt für alle Subjekte die rohen DAQ-Daten und die interpolierten CSVs,
    nutzt die interpolierte 'video'-Spalte zur Segmentierung, schneidet aus den
    DAQ-Signalen Fenster.
    ...
    for raw_path in raw_files:
        ...
        df_raw.columns = ["daqtime", "ecg", "bvp", "gsr", "rsp", "skt", "emg_zygo", "emg_coru", "emg_trap"]
        ...
        # Downsampling-Faktor für RAW
        ds_factor = compute_downsample_factor(t_raw_sec, cfg.target_fs)
        idx_raw_ds = np.arange(0, len(df_raw), ds_factor, dtype=int)

        t_raw_ds = t_raw_sec[idx_raw_ds]
        sig_raw_ds = df_raw[PHYSIO_COLS].to_numpy(dtype=float)[idx_raw_ds].T  # (C, T_total)
        ...
        # Segmente aus interpolierten Daten ermitteln (video konstant)
...
def refit_final_cnn(
    ...
    # finaler Fit: kein Val-Set → train_cnn_model ohne Early-Stopping-Logik
    model, hist = train_cnn_model(
        X_tr=X_norm,
        y_tr=y,
        X_va=None,
