...
- Mittelwert ± 1 SD pro Video über alle Subjekte
- Zeit-normalisiert (0–1), keine Amplituden-Normierung
...
SAVE_FIG = True
FIG_OUT = Path("skt_mean_timeline_by_video.png")
DPI = 200
...
TITLE = "Skin Temperature (SKT) — averaged per video across subjects (±1 SD)"
...
def normalize_segment(y: np.ndarray, n_points: int) -> np.ndarray:
    """Zeit-Normierung: Interpolation auf 0–1."""
    if y.size == 0:
        return np.array([])
...
# Daten sammeln
# ------------------------------------------------------------
data_by_video = {VIDEO_MAP[v]: [] for v in VIDEO_MAP}
...
for sid in SUBJECTS:
    ...
    try:
        ...
        print(f"[WARN] sub_{sid}.csv nicht ladbar: {e}")
        continue
...
for k, v in data_by_video.items():
    print(f"[INFO] {k}: N={len(v)}")
...
for vid_name, curves in data_by_video.items():
    ...
    if SHOW_INDIVIDUAL_CURVES:
        ...
        for i in idx:
            ax.plot(t, arr[i], color=color, alpha=0.25, lw=0.6)

    ax.plot(t, mean, lw=2.2, ls=ls, color=color, label=vid_name)
    ax.fill_between(t, mean - std, mean + std, color=color, alpha=0.15)

ax.grid(True, alpha=0.25)
ax.margins(x=0)
ax.legend(loc="upper left", bbox_to_anchor=(1.01, 1.02),
          title="Videos", frameon=False)
