# -*- coding: utf-8 -*-
"""
CASE Dataset — SKT: nur Individualkurven, Videos hintereinander (seriell)
...
Z_SCORE_PER_SEGMENT = False  # True = pro Segment z-standardisieren
SAVE_FIG = True
FIG_OUT = Path("skt_individual_serial_by_video.png")
DPI = 200
...
TITLE = "SKT (Skin Temperature) — individual curves only; videos in series (0–1 per segment)"
XLABEL = "Normalized time (segment blocks concatenated)"
YLABEL = "Skin Temperature (a.u.)"
...
for sid in SUBJECTS:
    ...
    missing = [c for c in ["daqtime", "skt", "video"] if c not in df.columns]
    if missing:
