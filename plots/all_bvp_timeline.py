# -*- coding: utf-8 -*-
"""
CASE Dataset — BVP: nur Individualkurven, Videos hintereinander (seriell)
...
Z_SCORE_PER_SEGMENT = False  # True, wenn du pro Segment z-standardisieren willst
SAVE_FIG = True
FIG_OUT = Path("bvp_individual_serial_by_video.png")
DPI = 200
...
LINESTYLE_BY_SUFFIX = {"1": "-", "2": "--"}  # wird hier nur für Trenner/Legende genutzt

TITLE = "BVP — Mean ± SD; videos in series (0–1 per segment)"
...
for sid in SUBJECTS:
    ...
    # Erwartete Spalten:
    missing = [c for c in ["daqtime", "bvp", "video"] if c not in df.columns]
    if missing:
    ...
    for vid_id in VIDEO_MAP:
        seg = df[df["video"] == vid_id]["bvp"].to_numpy()
        if seg.size == 0:
            continue
...
# x-Achse (Index 0..total_len-1) vorbereiten
...
for vid_id in PLOT_ORDER:
    ...
    x0 = starts[vid_id]
    xs = x[x0: x0 + block_len]  # Bereich für dieses Video
...
for i, vid_id in enumerate(PLOT_ORDER):
    x0 = starts[vid_id]
    # linker Randstrich (außer beim ersten Block)
    if i > 0:
...
leg1 = ax.legend(handles=handles, title="Kategorie", loc="upper left", bbox_to_anchor=(1.01, 1.02),
                 frameon=False)
