# =========================
# CASE Dataset GSR Feature Extraction with NeuroKit2 (extended & robust)
# =========================
# Requirements:
...
def _auc(y, dt):
    # Trapezoidal integral; returns "area under curve"
...
def _scr_morphology_from_sig(sig):
    ...
    out = {}
    # Common NK signal column names across versions
    # Peaks index column may be boolean mask or 0/1 vector
    for name in ["SCR_Amplitude", "SCR_RiseTime", "SCR_RecoveryTime", "SCR_Width", "SCR_Latency"]:
        if name in sig.columns:
            ...
            vals = vals[vals > 0] if vals.size else vals  # keep positive-only morphology
            if vals.size:
                out[name] = vals
    return out
...
def extract_eda_features(signal: pd.Series, sr: int) -> dict:
    ...
    if nk is not None:
        try:
            ...
            # Tonic mean/other stats if present in sig
            if "EDA_Tonic" in sig.columns:
                base.update(_basic_stats("EDA_Tonic", sig["EDA_Tonic"].values))
                base["EDA_Tonic_AUC"] = _auc(sig["EDA_Tonic"].values, dt)

            # Phasic stats if present
            if "EDA_Phasic" in sig.columns:
                base.update(_basic_stats("EDA_Phasic", sig["EDA_Phasic"].values))
                base["EDA_Phasic_AUC"] = _auc(sig["EDA_Phasic"].values, dt)

            # SCR rate per minute (durations from data length)
            n_peaks = None
            for cand in ["SCR_Peaks_N", "EDA_SCR_Peaks_N", "EDA_Peaks_N"]:
                if f"EDA_{cand}" in base:
                    try:
                        ...
                        pass
            # If not in base, try counting from signal's binary peak series
            if n_peaks is None:
                if "SCR_Peaks" in sig.columns:
                    # 'SCR_Peaks' may be a 0/1 vector; sum equals N peaks
            ...
            # Morphology aggregates if NK exposes arrays
