...
FEATURES = ["BVP_HR_Mean", "BVP_HR_SD", "BVP_Beat_Count", "BVP_Signal_SD"]
...
radar_features = ["BVP_HR_Mean", "BVP_HR_SD", "BVP_Signal_SD"]
...
for v in sorted(df["video"].unique()):
    values = norm_means.loc[v, :].tolist()
    values += values[:1]
...
print("\nSaved: per-video RADAR plots + category-level boxplots and bar charts in 'figures'.")
